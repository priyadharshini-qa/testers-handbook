...
A practical reference for QA engineers and testers working with  mySQL databases.
...
- Use `START TRANSACTION` / `ROLLBACK` to safely run `INSERT`, `UPDATE`, or `DELETE` during testing without committing changes.
...
*notes: all examples use MySQL syntax.*